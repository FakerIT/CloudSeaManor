...
void PixelToolbar::UpdateSlots(const std::vector<ToolbarSlot>& slots) {
    const int count = static_cast<int>(slots.size());
    const int copy_count = std::min(count, kSlotCount);
    for (int i = 0; i < copy_count; ++i) {
        slots_[i] = slots[i];
    }
    for (int i = copy_count; i < kSlotCount; ++i) {
        slots_[i] = ToolbarSlot{};
    }
    geometry_dirty_ = true;
}

void PixelToolbar::SetSelectedSlot(int index) {
    index = std::clamp(index, 0, kSlotCount - 1);
...
void PixelToolbar::MoveSelection(int delta) {
    if (delta == 0) return;
    const int next = (selected_slot_ + delta + kSlotCount) % kSlotCount;
