...
void PixelSpiritRealmPanel::RenderContent(sf::RenderWindow& window, const sf::FloatRect& inner_rect) {
    ...
    // 深层区域有毒云雾警告
    if (data_.current_layer >= 2 && !data_.toxic_cloud_warning.empty()) {
        sf::RectangleShape warning_bg({inner_rect.size.x - 24.0f, 28.0f});
        warning_bg.setPosition({x, y + row_h * 5.8f});
        warning_bg.setFillColor(sf::Color(120, 40, 40, 180));
        warning_bg.setOutlineThickness(1.0f);
        warning_bg.setOutlineColor(sf::Color(220, 80, 80, 255));
        window.draw(warning_bg);
        m_font_renderer->DrawText(window, data_.toxic_cloud_warning, {x + 6.0f, y + row_h * 6.0f}, TextStyle::Warning());
    }

    // 首领信息
    if (data_.current_layer >= 2 && !data_.boss_name.empty()) {
        m_font_renderer->DrawText(window, "【首领】" + data_.boss_name, {x, y + row_h * 7.0f}, TextStyle::Default());
        if (!data_.boss_hint.empty()) {
            m_font_renderer->DrawText(window, data_.boss_hint, {x, y + row_h * 7.5f}, TextStyle::Default());
        }
        if (data_.boss_defeated) {
            m_font_renderer->DrawText(window, "状态: 已击败 ✓", {x, y + row_h * 8.0f}, TextStyle::Success());
        }
    }

    sf::RectangleShape lock_hint({inner_rect.size.x - 24.0f, 24.0f});
    lock_hint.setPosition({x, y + row_h * 8.6f});
    ...
    m_font_renderer->DrawText(window, data_.lock_hint_text, {x + 6.0f, y + row_h * 8.85f}, TextStyle::HotkeyHint());

    m_font_renderer->DrawText(window, data_.actions_text, {x, y + row_h * 10.2f}, TextStyle::Default());
}
