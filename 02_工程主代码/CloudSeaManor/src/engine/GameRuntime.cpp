...
#define current_ambient_path_ state_.current_ambient_path
...
void GameRuntime::InitializeLoopCoordinator() {
    ...
    loop_coordinator_.RegisterSystem(
        ...
        [this](float delta_seconds) {
            const float recover_rate = world_state_.GetConfig().stamina_recover_per_second
    ...
    loop_coordinator_.RegisterSystem(
        ...
        [this]() {
            return world_state_.GetHunger().Current()
                < world_state_.GetHunger().Max();
        },
        2);
    ...
    loop_coordinator_.RegisterSystem(
        ...
        [this]() {
            // Buff系统在有active_buffs_时需要更新
            return true;  // BuffSystem::Tick会处理空情况
        },
        3);
    ...
    loop_coordinator_.RegisterSystem(
        ...
        [this]() {
            ...
            for (const auto& machine : machines) {
                if (machine.is_processing) return true;
            }
            return false;
        },
        1);
...
std::string WeatherAmbientPath_(CloudSeamanor::domain::CloudState weather) {
    using CloudSeamanor::domain::CloudState;
    switch (weather) {
    case CloudState::Rain: return "assets/audio/ambient/rain.wav";
    case CloudState::Mist: return "assets/audio/ambient/wind_mist.wav";
    case CloudState::HeavyCloud: return "assets/audio/ambient/wind_strong.wav";
    case CloudState::Tide: return "assets/audio/ambient/tide_magic.wav";
    default: return "";  // 晴天无环境音
    }
}
} // namespace
...
void GameRuntime::Initialize(
    ...
    // P18-QUAL-003: 设置契约奖励回调，处理 item/buff/title 奖励发放
    domain::ContractSystem::Instance().SetOnRewardGrant(
        [this](const std::string& reward_type, const std::string& reward_id, int count) {
            auto& inv = world_state_.MutableInventory();
            if (reward_type == "item" && count > 0) {
                inv.AddItem(reward_id, count);
                callbacks_.push_hint("获得物品：" + reward_id + " x" + std::to_string(count), 2.0f);
            } else if (reward_type == "buff" && count > 0) {
                world_state_.MutablePlayer().AddBuff(reward_id, count);
                callbacks_.push_hint("获得增益：" + reward_id + "（" + std::to_string(count) + "天）", 2.0f);
            }
        });
    ...
    // 加载成就奖励配置
    const std::string resolved_achievement_reward_path =
        services_.resource_manager.ResolveDataPath("achievement_rewards.csv").generic_string();
    if (!services_.achievement_rewards.LoadFromCsv(resolved_achievement_reward_path)) {
        CloudSeamanor::infrastructure::Logger::Warning(
            "GameRuntime: achievement_rewards.csv 加载失败，将使用硬编码奖励");
    }

    // 使用配置驱动的奖励发放函数
    auto grant_achievement_reward = services_.achievement_rewards.MakeGrantFunction(
        world_state_,
        [this](const std::string& msg, float duration) { callbacks_.push_hint(msg, duration); }
    );
...
void GameRuntime::OnDayChanged() {
    ...
    // 日终成就评估：使用配置驱动的奖励发放函数
    auto daily_grant_reward = services_.achievement_rewards.MakeGrantFunction(
        world_state_,
        [this](const std::string& msg, float duration) { callbacks_.push_hint(msg, duration); }
    );
    ...
    // BE-075: 天气环境音路由：雨天/薄雾/浓云/大潮切换环境音
    std::string ambient_path = WeatherAmbientPath_(systems_.GetCloud().CurrentState());
    if (ambient_path != current_ambient_path_) {
        if (callbacks_.play_sfx) {
            if (!ambient_path.empty()) {
                callbacks_.play_sfx(ambient_path);
            }
        }
        current_ambient_path_ = ambient_path;
    }
...
void GameRuntime::RestoreWorldBGM_() {
    // BE-075: 退出战斗时恢复世界BGM
    std::string bgm_path = SeasonalBgmPath_(world_state_.MutableClock().Season());
    const auto* today_festival = systems_.GetFestival().GetTodayFestival();
    if (today_festival != nullptr) {
        bgm_path = "assets/audio/bgm/festival_theme.wav";
    } else if (systems_.GetCloud().CurrentState() == CloudSeamanor::domain::CloudState::Tide) {
        bgm_path = "assets/audio/bgm/tide_theme.wav";
    ...
    if (callbacks_.play_bgm) {
        callbacks_.play_bgm(bgm_path, true, 0.8f, 0.3f);
    }
    current_bgm_path_ = bgm_path;
}
    ...
    for (const auto& object : world_state_.MutableInteractables()) {
        ...
        if (battle_manager_.ShouldTriggerBattle("spirit_beast", pos.x, pos.y, player_pos.x, player_pos.y)) {
            ...
            if (battle_manager_.EnterBattle(zone, enemies)) {
                ...
                // BE-075: 接入战斗BGM - 进入战斗时切换到战斗BGM
                if (callbacks_.play_bgm) {
                    const bool is_boss_battle = tide_boss_ready || zone_id == "zone_spirit_realm_3";
                    const std::string battle_bgm = is_boss_battle
                        ? "assets/audio/bgm/battle_boss.wav"
                        : "assets/audio/bgm/battle_normal.wav";
                    callbacks_.play_bgm(battle_bgm, true, 0.3f, 0.5f);
                }
                return true;
            }
        }
...
void GameRuntime::UpdateTimePhase_(float delta_seconds) {
    ...
    const float stamina_recover_rate = world_state_.GetConfig().stamina_recover_per_second
        * world_state_.GetHunger().StaminaRecoveryMultiplier()
        * world_state_.GetBuffs().StaminaRecoveryMultiplier();
