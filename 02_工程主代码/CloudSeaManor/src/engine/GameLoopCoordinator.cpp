...
GameLoopCoordinator::GameLoopCoordinator() {
    // 初始化阶段统计，使用 std::vector 而非 unordered_map
    phase_stats_.resize(static_cast<size_t>(LoopPhase::Count));
    for (int i = 0; i < static_cast<int>(LoopPhase::Count); ++i) {
        auto phase = static_cast<LoopPhase>(i);
        phase_stats_[static_cast<size_t>(i)].phase_name = GetPhaseName(phase);
    }
}
...
void GameLoopCoordinator::Update(float delta_seconds) {
    ...
    // 按阶段顺序执行所有系统
    for (const auto phase : GetPhaseOrder()) {
...
void GameLoopCoordinator::UpdatePhase(LoopPhase phase, float delta_seconds) {
    ...
    auto& phase_stat = phase_stats_[static_cast<size_t>(phase)];
    if (config_.enable_profiling) {
...
const PhaseStats& GameLoopCoordinator::GetPhaseStats(LoopPhase phase) const {
    ...
    if (index < phase_stats_.size()) {
        return phase_stats_[index];
    }
    return empty;
}
