...
void SystemLoopRegistry::RegisterTimeSystems_(
    ...
    coordinator.RegisterSystem(
        ...
        kStaminaSystem,
        [&world_state](float delta_seconds) {
            const float recover_rate = world_state.GetConfig().stamina_recover_per_second
    ...
    coordinator.RegisterSystem(
        ...
        [&world_state]() {
            // 饥饿度未满时更新
            return world_state.GetHunger().Current() < world_state.GetHunger().Max();
        },
        2);
