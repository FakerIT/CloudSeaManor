...
// ============================================================================
// 【SpiritBeastSystem::UpdateObstacleCache_】更新障碍物缓存
// ============================================================================
void SpiritBeastSystem::UpdateObstacleCache_(const GameWorldState& world_state) {
    const auto& obstacles = world_state.GetObstacleBounds();
    const std::size_t current_count = obstacles.size();

    // 仅在障碍物数量变化时重建缓存
    if (current_count != cached_obstacle_count_) {
        cached_obstacles_.clear();
        cached_obstacles_.reserve(current_count);
        for (const auto& obstacle : obstacles) {
            cached_obstacles_.push_back(obstacle);
        }
        cached_obstacle_count_ = current_count;
    }
}

// ============================================================================
// 【SpiritBeastSystem::IsInCachedObstacle_】检查点是否在缓存的障碍物中
// ============================================================================
bool SpiritBeastSystem::IsInCachedObstacle_(const sf::Vector2f& point) const {
    for (const auto& obstacle : cached_obstacles_) {
...
// ============================================================================
// 【SpiritBeastSystem::UpdateFollow_】灵兽跟随状态更新（使用障碍物缓存）
// ============================================================================
void SpiritBeastSystem::UpdateFollow_(
    GameWorldState& world_state,
    float delta_seconds
) {
    // P18-PERF-002: 更新障碍物缓存
    UpdateObstacleCache_(world_state);
    ...
    if (distance > elastic_keep_distance) {
        ...
        // P18-PERF-002: 使用缓存的障碍物数据，避免每帧重复调用 GetObstacleBounds()
        if (IsInCachedObstacle_(next_pos)) {
