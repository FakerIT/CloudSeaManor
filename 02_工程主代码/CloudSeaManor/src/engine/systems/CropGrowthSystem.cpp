...
// ============================================================================
void CropGrowthSystem::SetEcologySystem(const CloudSeamanor::domain::ManorEcologySystem* ecology) {
    ecology_system_ = ecology;
}

// ============================================================================
// 【CropGrowthSystem::CalculateGrowthMultiplier】计算生长倍率（带缓存优化）
// ============================================================================
float CropGrowthSystem::CalculateGrowthMultiplier(
    const TeaPlot& plot,
    const CloudSeamanor::domain::CloudState cloud_state
) {
    // P18-P003: 尝试使用缓存（仅当云海状态未变化时）
    const bool use_cache = plot.fertilized == false
        && plot.in_greenhouse == false
        && plot.sprinkler_installed == false;

    if (use_cache && growth_multiplier_cache_.valid
        && growth_multiplier_cache_.cloud_state == cloud_state) {
        return growth_multiplier_cache_.cached_multiplier;
    }
    ...
    if (use_cache) {
        growth_multiplier_cache_.cloud_state = cloud_state;
        growth_multiplier_cache_.cached_multiplier = growth_multiplier;
        growth_multiplier_cache_.valid = true;
    }
