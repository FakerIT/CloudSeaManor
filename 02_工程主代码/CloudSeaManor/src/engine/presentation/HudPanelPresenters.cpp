...
// ============================================================================
// 【CreateDefaultHeartLevels】创建默认心等级信息
// ============================================================================
std::vector<HeartLevelInfo> CreateDefaultHeartLevels() {
    static const std::vector<std::pair<int, std::string>> kHeartLevelNames = {
        {0, "陌生"}, {1, "初识"}, {2, "相识"}, {3, "熟识"}, {4, "好感"},
        {5, "亲近"}, {6, "亲密"}, {7, "挚友"}, {8, "知心"}, {9, "心许"}, {10, "永结"}
    };
...
void HudPanelPresenters::UpdateSpiritRealmPanel(PixelGameHud& hud, GameRuntime& runtime) {
    ...
    spirit_realm_view.current_layer = world_state.GetSpiritRealmLayer();
    ...
    // 深层特有信息
    if (spirit_realm_view.current_layer >= 2) {
        spirit_realm_view.toxic_cloud_warning = "⚠ 深层有毒云雾，每秒损失体力";
        spirit_realm_view.boss_name = "首领: 暗影君主";
        spirit_realm_view.boss_hint = "击败首领可获得稀有材料和大量经验";
    }
...
// ============================================================================
// 【UpdateSocialPanel】社交面板更新
// ============================================================================
void HudPanelPresenters::UpdateSocialPanel(PixelGameHud& hud, GameRuntime& runtime) {
    const auto& world_state = runtime.WorldState();
    const auto& npcs = world_state.GetNpcs();

    SocialPanelViewData social_view;
    social_view.title = runtime.Config().GetString("social_panel_title", "社交面板");
    social_view.filter_mode = "全部";
    social_view.sort_mode = "好感度";

    int total_friends = 0;
    int total_hearts_10 = 0;

    for (const auto& npc : npcs) {
        NpcSocialInfo npc_info;
        npc_info.npc_id = npc.id;
        npc_info.npc_name = npc.display_name;
        npc_info.favor = npc.favor;
        npc_info.heart_level = npc.heart_level;
        npc_info.talked_today = npc.talked_today;
        npc_info.gifted_today = npc.daily_gifted;
        npc_info.current_location = npc.current_location;

        // 获取喜好物品
        if (!npc.prefs.loved.empty()) {
            npc_info.loved_gifts = npc.prefs.loved[0];
            for (size_t i = 1; i < npc.prefs.loved.size() && i < 3; ++i) {
                npc_info.loved_gifts += ", " + npc.prefs.loved[i];
            }
        }

        // 获取讨厌物品
        if (!npc.prefs.disliked.empty()) {
            npc_info.disliked_gifts = npc.prefs.disliked[0];
            for (size_t i = 1; i < npc.prefs.disliked.size() && i < 3; ++i) {
                npc_info.disliked_gifts += ", " + npc.prefs.disliked[i];
            }
        }

        // 创建心等级进度
        npc_info.heart_levels = CreateDefaultHeartLevels();
        for (auto& level : npc_info.heart_levels) {
            level.unlocked = (level.level <= npc.heart_level);
        }

        // 计算距离下一级的进度
        int next_level_threshold = 0;
        if (npc.heart_level < 4) {
            next_level_threshold = (npc.heart_level + 1) * 20;
        } else if (npc.heart_level < 7) {
            next_level_threshold = 80 + (npc.heart_level - 3) * 50;
        } else if (npc.heart_level < 10) {
            next_level_threshold = 280 + (npc.heart_level - 6) * 100;
        } else {
            next_level_threshold = 680;
        }
        npc_info.favor_to_next_level = std::max(0, next_level_threshold - npc.favor);

        // 统计
        if (npc.heart_level >= 8) total_friends++;
        if (npc.heart_level >= 10) total_hearts_10++;

        // 添加已触发的事件（如果有）
        // 简化：使用心等级作为已触发事件数量
        for (int i = 1; i <= npc.heart_level && i <= 10; ++i) {
            npc_info.triggered_events.push_back("H" + std::to_string(i));
        }

        // 添加下一事件提示
        if (npc.heart_level < 10) {
            int next_event_level = npc.heart_level + 1;
            npc_info.next_event_hint = "好感达到" +
                std::to_string(next_event_level < 4 ? next_event_level * 20 :
                              next_event_level < 7 ? 80 + (next_event_level - 3) * 50 :
                              280 + (next_event_level - 6) * 100) +
                "可触发H" + std::to_string(next_event_level);
        }

        social_view.npcs.push_back(std::move(npc_info));
    }

    social_view.total_friends = total_friends;
    social_view.total_hearts_10 = total_hearts_10;

    hud.UpdateSocialPanel(social_view);
}
