...
// ============================================================================
// 【PixelGameHud::ToggleSocial】社交面板
// ============================================================================
void PixelGameHud::ToggleSocial() {
    if (panel_state_.dialogue_open) return;

    panel_state_.social_open = !panel_state_.social_open;
    social_panel_.SetVisible(panel_state_.social_open);
    if (panel_state_.social_open) {
        social_panel_.FadeIn();
        EmitUiEvent_(UiEventType::Open);
    } else {
        social_panel_.FadeOut();
        EmitUiEvent_(UiEventType::Close);
    }
    ...
    if (panel_state_.social_open) {
        ...
        panel_state_.map_open = false;
        minimap_.SetVisible(false);
        panel_state_.npc_schedule_open = false;
        npc_schedule_panel_.SetVisible(false);
    }
}
...
void PixelGameHud::UpdateSocialPanel(const SocialPanelViewData& data) {
    social_panel_.UpdateData(data);
}
...
bool PixelGameHud::HandleKeyPressed(const sf::Event::KeyPressed& key) {
    ...
    if (key.scancode == sf::Keyboard::Scancode::S) { ToggleSocial(); return true; }
    if (panel_state_.social_open) {
        if (key.scancode == sf::Keyboard::Scancode::Up) {
            social_panel_.SelectPrevNpc();
            return true;
        }
        if (key.scancode == sf::Keyboard::Scancode::Down) {
            social_panel_.SelectNextNpc();
            return true;
        }
    }
