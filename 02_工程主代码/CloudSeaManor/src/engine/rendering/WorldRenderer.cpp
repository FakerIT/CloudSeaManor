...
void WorldRenderer::RenderGroundTiles_(
    ...
    if (ws.GetInSpiritRealm()) {
        ...
        // 灵界深层：暗色调增强
        if (ws.GetSpiritRealmLayer() >= 2) {
            c = sf::Color(30, 25, 45, 120); // 深层：深紫黑色调
        }
    ...
    if (fr.flower_bloom_visual_day == gday) {
        // 使用对象池获取形状
        sf::RectangleShape& tint = rect_pool_.Acquire();
        tint.setSize({1280.0f, 720.0f});
        tint.setFillColor(sf::Color(255, 190, 210, 28));
        window.draw(tint);
    }
    if (fr.winter_solstice_polar_anchor_day == gday) {
        // 使用对象池获取形状
        sf::RectangleShape& polar = rect_pool_.Acquire();
        polar.setSize({1280.0f, 720.0f});
...
void WorldRenderer::RenderTeaPlots_(
    ...
    for (std::size_t i = 0; i < count; ++i) {
        ...
        if (ws.GetPurifyReturnDays() > 0) {
            ...
            // 使用对象池获取形状
            sf::CircleShape& spirit_dot = circle_pool_.Acquire();
            spirit_dot.setRadius(3.0f);
        ...
        if (plot.ready || plot.stage >= 4) {
            ...
            if (plot.quality >= CloudSeamanor::domain::CropQuality::Fine) {
                ...
                // 使用对象池获取形状
                sf::CircleShape& quality_glow = circle_pool_.Acquire();
                quality_glow.setRadius(crop_size.x * 0.7f);
            ...
            // 使用对象池获取形状
            sf::CircleShape& particle = circle_pool_.Acquire();
            particle.setRadius(2.5f);
...
void WorldRenderer::RenderParticles_(
    ...
    for (const auto& h : ws.GetHeartParticles()) {
        // 使用对象池获取形状，避免每帧创建
        sf::CircleShape& particle = circle_pool_.Acquire();
        particle.setRadius(h.radius);
        particle.setPointCount(CloudSeamanor::GameConstants::SpiritBeast::ParticlePointCount);
