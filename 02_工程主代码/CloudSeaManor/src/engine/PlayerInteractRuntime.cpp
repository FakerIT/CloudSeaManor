...
// Use domain types in this file
using CloudSeamanor::domain::PlayerMemoryType;
using CloudSeamanor::domain::PlayerMemorySystem;
...
// ============================================================================
// P18-QUAL-002: 表驱动工具效率配置
// ============================================================================
// 消除镰刀/斧头/镐/剪刀各一套重复逻辑，使用统一查询表
// ============================================================================

enum class ToolCategory { Sickle, Axe, Pickaxe, Scissors };

struct ToolEfficiencyEntry {
    const char* item_id;
    float efficiency;
};
...
constexpr ToolEfficiencyEntry kToolEfficiencySickle[] = {
    ...
    {"sickle_gold", 1.40f},
    {"sickle_spirit", 1.60f},
    {"SpiritSickle", 1.30f},  // 旧版灵气镰刀（向后兼容）
};

constexpr ToolEfficiencyEntry kToolEfficiencyAxe[] = {
    {"axe_copper", 1.20f},
    {"axe_silver", 1.40f},
    {"axe_gold", 1.60f},
    {"axe_spirit", 2.00f},
...
constexpr ToolEfficiencyEntry kToolEfficiencyPickaxe[] = {
    {"pickaxe_copper", 1.20f},
    {"pickaxe_silver", 1.40f},
    {"pickaxe_gold", 1.60f},
    {"pickaxe_spirit", 2.00f},
};

constexpr ToolEfficiencyEntry kToolEfficiencyScissors[] = {
    {"scissors_copper", 1.15f},
    {"scissors_silver", 1.20f},
    {"scissors_gold", 1.35f},
    {"scissors_spirit", 1.50f},
};

// ----------------------------------------------------------------------------
// GetBestToolEfficiency: 统一查询工具效率
// ----------------------------------------------------------------------------
float GetBestToolEfficiency_(
    const CloudSeamanor::domain::Inventory& inventory,
    ToolCategory category) {
    float best_efficiency = 1.0f;

    const ToolEfficiencyEntry* entries = nullptr;
    std::size_t count = 0;

    switch (category) {
    case ToolCategory::Sickle:
        entries = kToolEfficiencySickle;
        count = std::size(kToolEfficiencySickle);
        break;
    case ToolCategory::Axe:
        entries = kToolEfficiencyAxe;
        count = std::size(kToolEfficiencyAxe);
        break;
    case ToolCategory::Pickaxe:
        entries = kToolEfficiencyPickaxe;
        count = std::size(kToolEfficiencyPickaxe);
        break;
    case ToolCategory::Scissors:
        entries = kToolEfficiencyScissors;
        count = std::size(kToolEfficiencyScissors);
        break;
    default:
        return 1.0f;
    }
    ...
    for (std::size_t i = 0; i < count; ++i) {
        if (inventory.CountOf(entries[i].item_id) > 0) {
            best_efficiency = std::max(best_efficiency, entries[i].efficiency);
        }
    }
...
// 社交系统常量（可配置化）
// ============================================================================
constexpr int kDailyGiftTotalLimit = 3;              // 每日可赠送礼物总数
constexpr int kDailyFavorGainPerNpcLimit = 50;    // 每日对单个NPC好感度上限
constexpr int kMoodFavorMultiplierPercent = 100;    // 心情对好感影响倍率（100=1倍）
constexpr int kMoodDecayPerDay = 5;               // 心情每日衰减

// 云生特例配置
constexpr int kYunshengHomelessStage = 0;
constexpr std::string_view kYunshengHomelessPreferredGift = "Feed";
constexpr std::string_view kYunshengNormalGift = "TeaPack";
...
int ApplyNpcFavorDelta(NpcActor& npc, int delta) {
    // P13-GAP-003: 每日好感上限使用常量
    constexpr int kDailyFavorCap = kDailyFavorGainPerNpcLimit;
    // P13-GAP-003: 心情倍率使用配置化常量
    auto MoodFavorMultiplier_ = [](NpcMood mood) -> float {
        switch (mood) {
        case NpcMood::Sad:
        case NpcMood::Angry:
            // 心情不好时好感减半（使用kMoodFavorMultiplierPercent配置）
            return (kMoodFavorMultiplierPercent == 100) ? 0.5f : (kMoodFavorMultiplierPercent / 200.0f);
        case NpcMood::Happy:
        case NpcMood::Normal:
        default:
            return (kMoodFavorMultiplierPercent == 100) ? 1.0f : (kMoodFavorMultiplierPercent / 100.0f);
        }
    };
    if (delta > 0) {
...
void TryAddRelationshipMilestoneMemory(
    ...
    if (new_heart_level >= 4 && old_heart_level < 4) {
        ...
        ctx.memory_system->AddMemory(
            PlayerMemoryType::RelationshipMilestone,
            npc.id + "_friend",
            ctx.current_day,
            5,
            30
...
// ============================================================================
// 【ObstacleHitPowerFromEfficiency】障碍物击打次数
// ============================================================================
...
bool HandleGiftInteraction(PlayerInteractRuntimeContext& ctx) {
    ...
    if (gifted_today_count >= kDailyGiftTotalLimit) {
        ctx.dialogue_text = "今天已经送出 " + std::to_string(kDailyGiftTotalLimit) + " 份礼物了，明天再来吧~";
        ...
        std::string gift_item_id = std::string(kYunshengNormalGift);
        // P13-GAP-003: 云生流浪阶段偏好食物类赠礼，使用配置化常量
        if (npc.id == "yunseng" && npc.development_stage == kYunshengHomelessStage && !ctx.inventory.HasItem(std::string(kYunshengNormalGift))) {
            gift_item_id = std::string(kYunshengHomelessPreferredGift);
        }
        if (!ctx.inventory.TryRemoveItem(gift_item_id, 1)) {
            if (gift_item_id == std::string(kYunshengHomelessPreferredGift)) {
                ctx.dialogue_text = "你需要 " + std::string(kYunshengNormalGift) + " x1（或" + std::string(kYunshengHomelessPreferredGift) + " x1）才能给现在的云生送礼。";
            } else {
                ctx.dialogue_text = "你需要 " + std::string(kYunshengNormalGift) + " x1 才能送礼。";
            }
            ctx.push_hint(ctx.dialogue_text, 2.6f);
            UpdateUiAfterInteraction(ctx);
            return true;
        ...
        // P13-GAP-003: 使用配置化常量处理云生特例
        if (gift_item_id == std::string(kYunshengHomelessPreferredGift)
            && npc.id == "yunseng"
            && npc.development_stage == kYunshengHomelessStage) {
            favor_change = 20;
...
bool HandlePrimaryInteraction(PlayerInteractRuntimeContext& ctx) {
    ...
    if (ctx.highlighted_npc_index >= 0) {
        ...
        if (ctx.dialogue_manager && ctx.dialogue_nodes && ctx.dialogue_start_id) {
            // 检查婚姻状态
            const bool is_married = (ctx.relationship_state.stage == CloudSeamanor::domain::RelationshipStage::Married);
            const std::string spouse_id = is_married ? ctx.relationship_state.target_npc_id : "";
            const std::string spouse_call = ""; // 未来扩展：玩家自定义配偶称呼
    ...
    if (ctx.highlighted_plot_index >= 0) {
        ...
        if (!plot.cleared) {
            ...
            case PlotObstacleType::Stone: {
                ...
                const int hit_power = ObstacleHitPowerFromEfficiency_(
                    GetBestToolEfficiency_(ctx.inventory, ToolCategory::Pickaxe));
            ...
            case PlotObstacleType::Stump: {
                ...
                const int hit_power = ObstacleHitPowerFromEfficiency_(
                    GetBestToolEfficiency_(ctx.inventory, ToolCategory::Axe));
                plot.obstacle_hits_left = std::max(0, plot.obstacle_hits_left - hit_power);
                if (plot.obstacle_hits_left <= 0) {
                    cleared_now = true;
                } else {
        ...
        if (!plot.tilled) {
            ...
            // 镰刀收割效率加成
            const float sickle_efficiency = GetBestToolEfficiency_(ctx.inventory, ToolCategory::Sickle);
    ...
    if (ctx.highlighted_index >= 0 && ctx.stamina.Current() >= ctx.stamina_interact_cost) {
        ...
        case CloudSeamanor::domain::InteractableType::GatheringNode: {
            // 剪刀采集效率加成
            const float scissors_efficiency = GetBestToolEfficiency_(ctx.inventory, ToolCategory::Scissors);
