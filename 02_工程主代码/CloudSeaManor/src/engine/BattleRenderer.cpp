...
void BattleRenderer::RenderAmbientParticles_(sf::RenderWindow& window) {
    for (const auto& p : ambient_particles_) {
        // 使用对象池获取形状，避免每帧创建
        auto& shape = circle_pool_.Acquire();
        shape.setRadius(p.radius);
        shape.setPosition({p.x - p.radius, p.y - p.radius});
        shape.setFillColor(sf::Color(p.r, p.g, p.b, static_cast<std::uint8_t>(p.alpha * 255.0f)));
        window.draw(shape);
        // 形状在循环结束后自动销毁，无需手动释放（因为是引用）
        // 注：如果需要更严格的内存管理，可以使用 AcquireUnique/Release 模式
    }
}
