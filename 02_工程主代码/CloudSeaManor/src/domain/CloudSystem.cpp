...
std::string CloudSystem::ToHint(CloudState state) {
    ...
    case CloudState::Clear:
        return "今日晴空，基础正常生长";
    case CloudState::Mist:
        // 注意：薄雾仅影响生长速度，不影响作物品质（品质由 harvest 时快照决定）
        return "薄雾缭绕，作物生长+15%（品质提升需大潮/生态加成）";
    case CloudState::DenseCloud:
        return "浓云海降临，作物生长+40%，稀有灵茶/灵兽出现率+50%";
    case CloudState::Tide:
        // 代码实现：CropGrowthSystem 使用 ×1.60（+60%），WorkshopSystem 使用 speed = 1.0 + density * 0.5 = ×1.50（+50%）
        // 统一说明：作物生长+60%，工坊加工速度+50%，掉落+60%
        return "大潮涌动，作物生长+60%，工坊加工+50%，掉落+60%！";
    }
    return "";
}
