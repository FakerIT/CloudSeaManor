...
void FestivalSystem::Update(CloudSeamanor::domain::Season season, int day_in_season) {
    // 查找今日节日
    // P14-BLOCK-004 修复：支持同日多节日处理
    today_festival_id_.clear();
    std::vector<const Festival*> today_festivals;

    for (const auto& fest : festivals_) {
        if (fest.season == static_cast<int>(season) && fest.day == day_in_season) {
            today_festivals.push_back(&fest);
        }
    }

    if (!today_festivals.empty()) {
        // 节日优先级处理：云海大潮 > 特殊节日 > 普通节日
        // 如果有多个节日，选择优先级最高的
        const Festival* priority_festival = nullptr;
        for (const auto* fest : today_festivals) {
            if (!priority_festival) {
                priority_festival = fest;
            } else {
                // CloudTide 类型优先级最高
                if (fest->type == FestivalType::CloudTide && priority_festival->type != FestivalType::CloudTide) {
                    priority_festival = fest;
                }
            }
        }
        if (priority_festival) {
            today_festival_id_ = priority_festival->id;
        }

        // 如果有多个节日，记录冲突日志（供UI提示）
        if (today_festivals.size() > 1) {
            infrastructure::Logger::Info(
                "FestivalSystem: 同日 " + std::to_string(static_cast<int>(season))
                + " 第" + std::to_string(day_in_season) + " 天有 "
                + std::to_string(today_festivals.size()) + " 个节日冲突，优先显示："
                + today_festival_id_);
        }
    }
    ...
    for (const auto& fest : festivals_) {
        // 计算到节日的天数（考虑跨年情况）
        // 每个季节28天，一年共112天
        int fest_season = fest.season;
        int current_season_val = static_cast<int>(season);

        // 计算季节差值（支持跨年）
        int season_diff = fest_season - current_season_val;
        if (season_diff < 0) {
            season_diff += 4;  // 跨年：假设 festival 在去年
        }

        // 计算总天数
        int days_until = season_diff * DAYS_PER_SEASON + (fest.day - day_in_season);

        // 只考虑未来的节日（days_until > 0）
