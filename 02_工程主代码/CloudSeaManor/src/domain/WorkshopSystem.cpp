...
// 阶段总数（不包括 Idle 和 FinishedTea）
constexpr int TEA_STAGE_COUNT = 5;
...
std::vector<MachineCompletionInfo> WorkshopSystem::Update(
    ...
    // P13-GAP-002 修复：品质分数上限（防止数值无意义增长）
    constexpr float MAX_QUALITY_SCORE = 1000.0f;
    ...
    for (auto& machine : machines_) {
        ...
        // 动态阶段进度：根据总加工时间计算每个阶段的时长
        const float total_time = static_cast<float>(recipe->process_time);
        const float stage_time = total_time / TEA_STAGE_COUNT;
        const float stage_speed = (100.0f / stage_time) * delta_time * speed;

        machine.progress += (100.0f / total_time) * delta_time * speed;
        machine.stage_progress += stage_speed;
        ...
        // 品质分数累加（有上限）
        const float quality_gain = delta_time * std::max(0.0f, cloud_density) * 0.2f
            + static_cast<float>(std::max(0, skill_level - 1)) * 0.04f
            + static_cast<float>(std::max(0, tool_level - 1)) * 0.03f;
        machine.quality_score = std::min(machine.quality_score + quality_gain, MAX_QUALITY_SCORE);

        if (machine.progress >= 100.0f) {
            // P13-GAP-002 修复：成功率判定
            // 如果随机数大于 base_success_rate，加工失败，返还部分原料
            const float roll = static_cast<float>(std::rand()) / static_cast<float>(RAND_MAX);
            const float success_rate = recipe->base_success_rate;

            if (roll > success_rate) {
                // 加工失败：返还 50% 原料
                infrastructure::Logger::Info(
                    "WorkshopSystem: 加工失败 " + recipe->name +
                    "（成功率=" + std::to_string(static_cast<int>(success_rate * 100)) + "%）");
                output_items[recipe->input_item] += recipe->input_count / 2;
            } else {
                // 加工成功
            ...
            info.output_count = (roll <= success_rate) ? recipe->output_count : 0;
