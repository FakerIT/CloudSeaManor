...
- [x] `PlayerVisualComponent` 已独立建模并接入 engine 渲染组件
- [x] 本轮复核：`src/domain` 已完成 SFML 头文件清扫验收
...
- [x] `GameWorldState.cpp` / `UISystem.cpp` 全量硬编码扫描与迁移已验收（world_tip 动画参数迁移至配置）
- [x] 本轮复核：UI 魔法数字已完成“代码->配置”闭环（新增 `numbers.world_tip_wave_amplitude/period`）
...
- [x] 本轮新增 `TeaGardenTest` 与 `CloudGuardianContractTest`，补充 domain 覆盖面
- [x] 本轮新增 `CloudSystemTest` 与 `ShopSystemTest`，补充天气与交易核心分支覆盖
- [x] 本轮新增 `DialogueEngineTest`，补充分支路由/打字机/占位符替换覆盖
- [x] 本轮新增 `WorkshopSystemTest`，补充加工进度/产出/升级/槽位等核心分支覆盖
- [x] 本轮新增 `InteractionSystemTest`，补充地块/工坊/修缮/采集等交互核心分支覆盖
- [x] 本轮扩展 `CropGrowthSystemTest` / `FarmingLogicTest`，补充肥料/温室/洒水器/季节枯萎等生长分支覆盖
...
- [x] `raw land/obstacle` 子状态与工具耐久扣减链路已补齐（石头/树桩/杂草）
- [x] 本轮复核：障碍物子状态切换与耐久扣减已闭环验证
...
- [x] `spirit_beast_zone` / `spirit_gateway_return` 语义对象与交互联动已补齐
...
- [x] 已补齐稀有采集点显式生成（大潮日自动生成 `Spirit Plant Rare`）
- [x] `SpiritRealmManager` 独立系统已接入（稀有节点刷新从 `GameRuntime` 抽离）
...
- [x] 已按 0-20/20-50/50-80/80-95/95-100 区间建模（荒芜/苏醒/兴盛/繁华/太初）
...
- [x] J 键战斗触发与中层战斗流程已接入（`TryEnterBattleByPlayerPosition` + `BattleManager`）
...
- [x] `ShopSystem` 独立类与店铺 UI（`PixelShopPanel`）已接入 HUD 与交互流程
...
- [x] 已完成战斗管理器初始化路径接入
- [x] `GameApp::Run` 已对齐运行时协调架构（战斗初始化在 `GameRuntime/BattleManager` 生效）
...
- [x] 已具备撤退与胜利确认的状态切换入口
- [x] Q/W/E/R、Escape、X 按键映射与独立战斗渲染分支已对齐主循环
...
- [x] 战斗中存档结算、防刷战斗与读档恢复策略已接入（存档前撤退并重置战斗态）
...
- [x] 事件总线监听（收获/送礼/建造/灵兽）与通用发奖流水已落地
...
- [x] `TeaGarden` 独立类与“160秒周期 + 每周浇水”规则已固化
...
- [x] 家具类型已细分为桌/椅/床/书架/盆栽（摆放网格系统后续补充）
- [x] 舒适度对体力恢复速度的数值联动已接入（按日结算额外恢复）
...
**完成标识**：
- [x] 已具备畜棚喂养与产出相关交互基础
- [x] `CoopSystem/BarnSystem` 独立养殖循环已接入每日结算
...
- [x] “核心数据表从 mod 目录扩展加载”已落地到 Price/Crop/Dialogue/Map 接入点
...
- [x] 玩家输入命名 UI 入口已接入（`F2` 进入输入模式，Enter 确认）
...
- [x] 独立互动菜单 UI 已接入（G 打开菜单，1/2/3 选择，Enter/G 确认）
...
- [x] `PetSystem` 独立类已落地
- [x] 猫/狗/鸟差异化与跟随动画已接入
- [x] 宠物商品购买已支持选购菜单（1/2/3 选择 + Enter/E 确认）
...
- [x] 全类型灵兽收集里程碑已实现（按活泼/慵懒/好奇三类型结缘触发）
- [x] 成就通知横幅与灵兽羁绊专用事件链已统一到 `EventBus(beast_bond)`
