...
public:
    ...
    // 灵界层级：0=浅层(入口), 1=中层(潮汐裂谷), 2=深层(霜岚祭坛/首领区域)
    [[nodiscard]] int GetSpiritRealmLayer() const { return spirit_realm_layer_; }
    [[nodiscard]] int& MutableSpiritRealmLayer() { return spirit_realm_layer_; }
    void SetSpiritRealmLayer(int value) { spirit_realm_layer_ = std::clamp(value, 0, 2); }

    // 深层有毒云雾状态
    [[nodiscard]] float GetToxicCloudDamage() const { return toxic_cloud_damage_per_second_; }
    [[nodiscard]] bool IsInToxicCloud() const { return in_spirit_realm_ && spirit_realm_layer_ >= 2; }
...
private:
    ...
    int spirit_realm_layer_ = 0;  // 0=浅层, 1=中层, 2=深层
    float toxic_cloud_damage_per_second_ = 0.0f;  // 深层有毒云雾伤害
