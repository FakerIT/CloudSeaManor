...
struct ColorPalette {
    ...
    static constexpr sf::Color FreshGreen{50, 205, 50};         // LimeGreen #32CD32，新鲜
    static constexpr sf::Color FireOrange{255, 140, 0};         // DarkOrange #FF8C00，火焰橙
    static constexpr sf::Color FireRed{220, 20, 60};            // Crimson #DC143C，火焰红
    static constexpr sf::Color OceanBlue{70, 130, 180};      // SteelBlue #4682B4，海蓝
    static constexpr sf::Color StoneGray{128, 128, 128};     // Gray #808080，石头灰
...
public:
     ...
     * @param target 绘制目标
     * @param rect 按钮矩形
     * @param state 按钮状态（0=默认，1=悬停，2=按下，3=禁用）
     */
    void DrawPixelButton(sf::VertexArray& target,
                        const sf::FloatRect& rect,
                        int state = 0) const;
     ...
     * @brief 绘制像素风格进度条
     * @param target 绘制目标
     * @param rect 进度条矩形（整个背景区域）
     * @param fill_ratio 填充比例（0.0~1.0）
     * @param fill_color 填充颜色
     * @param show_border 是否显示边框
     */
    void DrawPixelProgressBar(sf::VertexArray& target,
                             const sf::FloatRect& rect,
                             float fill_ratio,
                             const sf::Color& fill_color,
                             bool show_border = true) const;
     ...
     */
    void DrawToolbarHighlight(sf::VertexArray& target,
                             const sf::FloatRect& rect,
     ...
     */
    void DrawPortraitFrame(sf::VertexArray& target,
                           const sf::FloatRect& rect,
                           const sf::Color& border_color = ColorPalette::BrownOutline) const;
