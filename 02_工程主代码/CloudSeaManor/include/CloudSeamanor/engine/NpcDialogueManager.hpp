...
struct DailyDialogueEntry {
    ...
    bool is_married = false;  // 是否为婚后专属对话（false=所有状态，true=仅已婚）
    bool is_spouse_only = false;  // 是否为配偶专属（需与ctx.spouse_id匹配）
};
...
struct NpcDialogueContext {
    ...
    // 婚后特殊变量
    bool is_married = false;  // 玩家是否已婚
    std::string spouse_call = "";  // 配偶对玩家的称呼（自定义）
    std::string spouse_id = "";  // 配偶NPC的ID
...
private:
    ...
    std::unordered_map<std::string, std::vector<StageDialogueEntry>> stage_dialogues_;
    ...
    bool stage_dialogues_loaded_ = false;
};
