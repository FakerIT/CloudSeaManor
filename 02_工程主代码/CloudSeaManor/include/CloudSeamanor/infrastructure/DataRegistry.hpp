...
public:
    ...
    void RegisterCsvTable(
        ...
        Parser parser) {
        // 捕获 table_id 到 lambda 中
        const std::string captured_table_id = table_id;
        csv_table_loaders_.push_back(CsvLoaderRegistration{
            .table_id = table_id,
            .relative_path = relative_path,
            .loader = [target_table, parser, this, captured_table_id](const CsvDocument& document,
                                             std::vector<DataIssue>& issues) {
                std::vector<T> rows;
                rows.reserve(document.rows.size());
                bool success = true;

                auto schema_it = schemas_.find(captured_table_id);
                if (schema_it != schemas_.end()) {
                    if (!DataTableValidation::CheckDuplicateIds(
                            document, schema_it->second.id_field, issues, captured_table_id,
                            GetRegisteredPath_(captured_table_id))) {
                        success = false;
                    }
                }

                const auto& valid_ids = GetExternalIds_(captured_table_id);

                for (const auto& row : document.rows) {
                    if (schema_it != schemas_.end() &&
                        (!schema_it->second.required_fields.empty() || !valid_ids.empty())) {
                        if (!DataTableValidation::ValidateRow(
                                row, schema_it->second, valid_ids, issues, captured_table_id,
                                GetRegisteredPath_(captured_table_id))) {
                            success = false;
                        }
                    }
                ...
                if (success && target_table != nullptr) {
                    ...
                    auto schema_it2 = schemas_.find(captured_table_id);
                    if (schema_it2 != schemas_.end()) {
                        external_id_sets_[captured_table_id] = target_table->GetAllIds();
                    }
                }
                return success;
            },
