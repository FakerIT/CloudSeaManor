...
struct ResourceStats {
    ...
    bool memory_usage_estimated = true;  // 标记内存估算值是否基于硬编码平均值
};
...
public:
    ...
    // ========================================================================
    // 【SpriteMapping 集成】通过逻辑 ID 获取纹理
    // ========================================================================

    /**
     * @brief 设置 SpriteMapping 映射表。
     * @param mapping 映射表引用。
     * @param base_path 资源根目录。
     * @note 在 LoadTextureBySpriteId 之前调用。
     */
    void SetSpriteMapping(const SpriteMapping* mapping, const std::string& base_path = "");

    /**
     * @brief 通过 SpriteId 加载纹理（查询映射表后加载）。
     * @param sprite_id 逻辑资源 ID（如 "npc_yun_001_idle"）。
     * @param texture_id 可选的纹理 ID（默认使用 sprite_id）。
     * @return true 表示加载成功。
     */
    bool LoadTextureBySpriteId(const std::string& sprite_id,
                                const std::string& texture_id = "");

    /**
     * @brief 通过 SpriteId 批量预加载纹理。
     * @param sprite_ids 逻辑资源 ID 列表。
     * @return 成功加载的数量。
     */
    size_t PreloadTexturesBySpriteIds(const std::vector<std::string>& sprite_ids);
...
private:
    ...
    // SpriteMapping 映射表
    const SpriteMapping* sprite_mapping_ = nullptr;
    std::string sprite_base_path_;
};
